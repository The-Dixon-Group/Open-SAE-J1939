...
uint8_t GetActiveErrors(J1939* j1939)
{
    uint8_t return_val = 0;
    
    for(uint8_t i = 0; i < MAX_DM_FIELD; i++)
    {
        if(j1939->this_dm.DTC[i].active)
            return_val++;
    }
...
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_DM1(J1939* j1939, uint8_t DA) {
	uint8_t errors_active = GetActiveErrors(j1939);
    if(errors_active < 2) {
		uint32_t ID = (0x18FECA << 8) | j1939->information_this_ECU.this_ECU_address;
		uint8_t data[8];
        memcpy(data, &(j1939->this_dm.DTC[0]),6);
		//data[0] = (j1939->this_dm.dm1.status_indicators.byte);// SAE_lamp_status_malfunction_indicator << 6) | (j1939->this_dm.dm1.SAE_lamp_status_red_stop << 4) | (j1939->this_dm.dm1.SAE_lamp_status_amber_warning << 2) | (j1939->this_dm.dm1.SAE_lamp_status_protect_lamp);
		//data[1] = (j1939->this_dm.dm1.flash_indicators.byte);// SAE_flash_lamp_malfunction_indicator << 6) | (j1939->this_dm.dm1.SAE_flash_lamp_red_stop << 4) | (j1939->this_dm.dm1.SAE_flash_lamp_amber_warning << 2) | (j1939->this_dm.dm1.SAE_flash_lamp_protect_lamp);
		...
		j1939->this_ecu_tp_cm.total_message_size_being_transmitted = (errors_active *4) +2 ;				/* set total message size where each DTC is 4 btyes, plus 2 bytes for the lamp code */
		...
		j1939->this_ecu_tp_dt.data[0] = (j1939->this_dm.status_indicators.byte); //SAE_lamp_status_malfunction_indicator << 6) | (j1939->this_dm.dm1.SAE_lamp_status_red_stop << 4) | (j1939->this_dm.dm1.SAE_lamp_status_amber_warning << 2) | (j1939->this_dm.dm1.SAE_lamp_status_protect_lamp);
		j1939->this_ecu_tp_dt.data[1] = (j1939->this_dm.flash_indicators.byte); //SAE_flash_lamp_malfunction_indicator << 6) | (j1939->this_dm.dm1.SAE_flash_lamp_red_stop << 4) | (j1939->this_dm.dm1.SAE_flash_lamp_amber_warning << 2) | (j1939->this_dm.dm1.SAE_flash_lamp_protect_lamp);
		...
		for (i = 0; i < j1939->this_ecu_tp_cm.total_message_size_being_transmitted; i++){
            ...
            //j1939->this_ecu_tp_dt.data[i*4 + 2] = j1939->this_dm.DTC[i] SPN[i];
